...
QueryX lists accessible PostgreSQL, SQL Server, and Oracle functions/procedures in each schema's **Routines** group. Selecting one opens its overload-safe signature, language, return shape, and the catalog metadata available for that callable without executing SQL. PostgreSQL also exposes aggregate and window-function entries; Oracle includes standalone routines and package methods.
...
1. Connect to PostgreSQL, SQL Server, or Oracle and expand **Schemas**.
2. Expand a schema, then **Routines**.
3. Select `name(arguments)`. The complete identity argument list distinguishes overloads.
4. Review **Routine details** and, when the driver provides it, **Database-rendered DDL** in the Inspector.
5. For a provided definition, choose **Copy DDL** to place the displayed text on the clipboard, or **Edit in SQL** to open a new editable tab. Neither action executes the definition.
...
Definitions for ordinary PostgreSQL functions/procedures come from `pg_get_functiondef`; SQL Server definitions come from `sys.sql_modules.definition`. Oracle currently reads signature metadata from `ALL_PROCEDURES` and `ALL_ARGUMENTS` but does not synthesize executable routine DDL. Database-rendered source/DDL is never claimed to preserve original authoring whitespace or comments.
...
Use **Run in Transaction** from the new SQL tab after reviewing or editing a provided statement. The transaction is rolled back when execution fails, and the SQL remains available for correction. Aggregate/window entries and Oracle routines do not offer this handoff when the driver has no executable definition panel.
...
- **An Oracle routine has no DDL panel:** Oracle signature and argument metadata are available, but QueryX does not invent a `CREATE OR REPLACE` definition until a safe source/DDL reconstruction contract is implemented.
