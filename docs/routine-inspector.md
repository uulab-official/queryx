...
QueryX lists accessible PostgreSQL functions, procedures, aggregates, and window functions in each schema's **Routines** group. Selecting one opens its overload-safe signature, language, return shape, and the catalog metadata available for that callable without executing SQL.
...
Definitions for ordinary functions and procedures come from `pg_get_functiondef`. PostgreSQL reconstructs a `CREATE OR REPLACE` statement, so whitespace, comments, and formatting from the original source may differ. QueryX labels this as database-rendered DDL rather than claiming it is the original authoring text.
