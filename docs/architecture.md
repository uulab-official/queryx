...
- Operations policy: native PostgreSQL and MySQL/MariaDB sessions and lock waits are inspected through the driver capability contract; redacted observation history is stored only in the local workspace boundary. Operations panels may cancel a running query, but never terminate a database connection or bypass server permissions.
...
6. Add graphical plan layout, cost controls, JSON plan contracts, and SQL Server/Oracle/SQLite execution-plan contracts on top of the current PostgreSQL/MySQL text-plan tree, `EXPLAIN ANALYZE` controls, and non-executing Explain slice documented in [ADR-0012](decisions/ADR-0012-non-executing-explain-slice.md).
