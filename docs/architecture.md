...
- Operations policy: native PostgreSQL and MySQL/MariaDB sessions and lock waits are inspected through the driver capability contract; the operations panels may cancel a running query, but never terminate a database connection or bypass server permissions.
