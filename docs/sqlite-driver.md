...
- `database_metadata` — returns schemas, tables, views, columns, indexes, foreign keys, triggers, and an explicit empty routines array for Explorer/Inspector
...
- SQLite metadata covers tables, views, columns, primary-key markers, indexes, ordered foreign-key pairs, and `main` relation triggers. Trigger DDL comes from `sqlite_master.sql`; timing/events are conservatively parsed and status is `enabled`. It returns `routines: []` because SQLite has no stored function/procedure catalog equivalent.
