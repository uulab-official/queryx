...
Virtualization reduces DOM work but does not reduce memory use: loaded pages remain available for local filtering and export. For PostgreSQL and MySQL/MariaDB connections, **Stream** runs a single safe `SELECT`/`WITH` query through the native row stream and appends 256-row chunks as they arrive; the normal **Cancel** action remains available while the stream is active. SQLite intentionally uses the buffered fallback and does not advertise the streaming capability yet. QueryX still falls back to normal execution for DML, multiple statements, locking clauses, and queries with unterminated SQL syntax. Chunk backpressure/disk spill, dedicated progress telemetry, and server-side filtering for arbitrary queries remain roadmap items.

Streaming is an accumulation path: received rows remain in the local result for filtering, copy, and export. It improves time-to-first-row and bounded native fetches, but it does not make an unbounded export memory-free. Use an explicit `LIMIT`, projection, and ordering for repeatable large-result workflows.
