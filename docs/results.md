...
Virtualization reduces DOM work but does not reduce memory use: loaded pages remain available for local filtering and export. For PostgreSQL connections, **Stream** runs a single safe `SELECT`/`WITH` query through a server-side cursor and appends 256-row chunks as they arrive; the normal **Cancel** action remains available while the stream is active. SQLite and MySQL/MariaDB intentionally use the buffered fallback and do not advertise the streaming capability yet. QueryX still falls back to normal execution for DML, multiple statements, locking clauses, and queries with unterminated SQL syntax. Chunk backpressure/disk spill, dedicated progress telemetry, and server-side filtering for arbitrary queries remain roadmap items.
