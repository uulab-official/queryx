...
Virtualization reduces DOM work but does not reduce memory use: loaded pages remain available for local filtering and export. For all native connections, **Stream** runs a single safe `SELECT`/`WITH` query through the native row stream and appends 256-row chunks as they arrive. PostgreSQL and MySQL/MariaDB keep the normal **Cancel** action available while the stream is active; SQLite streams without a cancellation control because SQLite does not expose a safe interrupt path through the current driver contract. QueryX still falls back to normal execution for DML, multiple statements, locking clauses, and queries with unterminated SQL syntax. Chunk backpressure/disk spill, dedicated progress telemetry, and server-side filtering for arbitrary queries remain roadmap items.
