...
QueryX can compare a captured metadata baseline with a later metadata refresh. This is a review workflow: it generates migration SQL but never executes it automatically.
...
The baseline lives in the active desktop session only. Switching connections clears it so a snapshot from one database cannot accidentally be compared with another.
