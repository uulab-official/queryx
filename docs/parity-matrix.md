...
| Session and lock explorer | Partial — native PostgreSQL/MySQL/MariaDB session list, point-in-time PostgreSQL/MySQL/MariaDB lock-wait graph, and threshold-based long-running query diagnostics with safe query cancellation; session history remains planned | Available | Available | Partial | Available | [session explorer](session-explorer.md), `longRunningDiagnostics.test.ts`, native driver tests |
