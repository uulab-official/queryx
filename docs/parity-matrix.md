...
| Result grid, filter, sort, copy, export | Partial — virtualized loaded pages and 100-row server paging for single SELECT/WITH queries; table-browser server filter/sort is available, while streaming and arbitrary-query server filtering remain pending | Available | Available | Available | Available | [results](results.md), `resultGrid.test.ts`, `queryPaging.test.ts`, `tableBrowse.test.ts` |
| Table browsing and keyed edits | Partial — SQLite/PostgreSQL/MySQL primary-key browsing with dialect-aware server filter/sort, deterministic pagination, conflict-aware cell updates, and guarded selected-row deletion; inserts and arbitrary-query identity pending | Available | Available | Available | Available | [results](results.md), `tableBrowse.test.ts`, `csvExport.test.ts`, edit-batch tests |
