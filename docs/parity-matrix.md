...
| Session and lock explorer | Partial — native PostgreSQL/MySQL/MariaDB session list plus point-in-time PostgreSQL lock graph and MySQL/MariaDB Performance Schema/InnoDB lock-wait graph, with safe query cancellation; session history remains planned | Available | Available | Partial | Available | [session explorer](session-explorer.md), native driver tests |
