...
| Session and lock explorer | Partial — native PostgreSQL/MySQL/MariaDB session list with active/idle/waiting state, wait-event visibility, refresh, and safe query cancellation; lock graph and history remain planned | Available | Available | Partial | Available | [session explorer](session-explorer.md), native driver tests |
