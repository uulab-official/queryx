...
| Result grid, filter, sort, copy, export | Partial — virtualized loaded pages, 100-row server paging for single SELECT/WITH queries, table-browser server filter/sort, and PostgreSQL cursor-backed 256-row streaming; cross-driver streaming and arbitrary-query server filtering remain pending | Available | Available | Available | Available | [results](results.md), `resultGrid.test.ts`, `queryPaging.test.ts`, `queryStream.test.ts`, `tableBrowse.test.ts` |
