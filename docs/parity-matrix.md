...
| Table browsing and keyed edits | Partial — SQLite/PostgreSQL/MySQL/SQL Server/Oracle metadata-safe browsing, with SQL Server/Oracle atomic edit batches; Oracle index/FK identity metadata remains pending | Available | Available | Available | Available | [results](results.md), [SQL Server driver](sqlserver-driver.md), [Oracle driver](oracle-driver.md), `tableBrowse.test.ts`, `tableRowInsert.test.ts`, `csvExport.test.ts`, edit-batch tests |
