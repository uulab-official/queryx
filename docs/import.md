...
6. Choose a duplicate-key policy: **Stop and rollback** (default) or **Ignore conflicting rows**.
7. Choose **Import N rows**. QueryX sends one INSERT statement per row through the native edit batch, which commits all rows together or rolls the batch back.

Empty fields become SQL `NULL`. Integer, numeric, boolean, date, and JSON mappings are validated before any SQL is sent. Identifiers and string values are quoted for the active PostgreSQL, MySQL/MariaDB, or SQLite dialect. Ignore-conflict mode emits `ON CONFLICT DO NOTHING`, `INSERT IGNORE`, or `INSERT OR IGNORE` for the selected driver.
...
- CSV and object-based JSON import are supported. Delimiter selection, column transforms, update/upsert policies, progress, and resumable batches remain planned.
