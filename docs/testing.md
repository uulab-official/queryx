...
8. **Manual smoke checks** cover the high-value UI path: connect state → run a SELECT returning more than 100 rows → confirm only the first page loads, use **Load next 100**, and verify rows append without changing the editor SQL or history entry → on PostgreSQL run a bounded large `SELECT`/`WITH` with **Stream**, confirm rows appear before completion, then cancel a long-running stream and verify the partial result is not presented as a completed result → browse a selected table, apply a server filter/order, use **New row** with a database default and a typed value, review the INSERT preview, select row numbers, review a DELETE preview, and confirm rollback behavior → filter/sort/copy result → export CSV/JSON/SQL INSERT → import a typed CSV into a selected table, exercise stop/ignore/upsert policies with mapped conflict keys, and confirm rollback behavior → open Create table, Add column, Edit columns, Create index, Drop index, Add foreign key, Drop foreign key, Create view, Edit definition of selected view, or Drop selected view from the command palette, preview SQL, apply a disposable change, and refresh metadata → inspect and navigate object dependencies.
