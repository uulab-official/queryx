...
8. **Manual smoke checks** cover the high-value UI path: connect state → run query → filter/sort/copy result → export CSV/JSON/SQL INSERT → import a typed CSV into a selected table and confirm batch rollback behavior → open Create table, Add column, Edit columns, Create index, Drop index, or Create view from the command palette, preview SQL, apply a disposable change, and refresh metadata → inspect and navigate object dependencies.
