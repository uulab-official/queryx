...
7. **Live database contract checks** execute PostgreSQL behavior against a disposable service and optionally exercise MySQL/MariaDB or Oracle when the corresponding environment is available.
8. **Manual smoke checks** cover the high-value UI path: connect state → run a SELECT returning more than 100 rows → confirm only the first page loads, change the filter/sort and choose **Apply to query**, verify the DB-backed result replaces the first page while the editor SQL/history keep the original query, then use **Load next 100** and confirm the same filter/order is retained → on each native driver run a bounded large `SELECT`/`WITH` with **Stream**, confirm rows appear before completion, then cancel a long-running PostgreSQL/MySQL stream while confirming SQLite remains non-cancellable → browse a selected table, apply a server filter/order, use **New row** with a database default and a typed value, review the INSERT preview, select row numbers, review a DELETE preview, and confirm rollback behavior → choose **Snapshot**, open the SQL file, verify the table name, dialect quoting, INSERT values, transaction wrapper, and complete/partial row marker → run a disposable high-risk `ALTER`/`DROP`/`TRUNCATE`, verify Safe Mode offers the loaded snapshot action and still requires explicit execution → filter/sort/copy result → export CSV/JSON/SQL INSERT/Markdown/standard `.xlsx` and open the workbook in a spreadsheet viewer, confirming formula-looking text remains text → import a typed CSV into a selected table, exercise stop/ignore/upsert policies with mapped conflict keys, and confirm rollback behavior → open Create table, Add column, Edit Columns, rename a column on a disposable PostgreSQL/MySQL/SQL Server/Oracle table, add UNIQUE and CHECK constraints on a disposable non-SQLite table, verify unsafe expressions and SQLite are manual-review only, Create index, Alter index columns/UNIQUE on a disposable non-primary index for PostgreSQL/MySQL/SQL Server/Oracle, verify the rebuild warning and primary/SQLite manual-review boundary, Drop index, Rename index with a non-primary index on PostgreSQL/MySQL/SQL Server/Oracle, verify primary/SQLite rename is manual-review only, Add foreign key, Drop foreign key, Create view, Edit definition form for a disposable routine/trigger on each supported native driver, verify missing/multi-statement definitions are rejected and SQLite is manual-review only, or Drop selected view from the command palette, preview SQL, apply a disposable change, and refresh metadata → inspect and navigate object dependencies.
...
The desktop preview also has a Safe Mode smoke path: replace the editor query with an UPDATE or DELETE without WHERE, press Cmd/Ctrl+Enter, confirm the warning, then choose Cancel, Run in Transaction, or Execute Anyway. Separately, choose **Begin**, run a write and a read in two queries, confirm the status bar shows **Transaction active**, then verify **Rollback** removes the write and **Commit** persists it. This is a UI contract preview; native transaction semantics belong to the Rust driver.

The editor smoke path creates a second query with Cmd/Ctrl+T, enters distinct SQL in both tabs, switches between them, and confirms that each document and its undo history remain independent. Selecting SQL before Cmd/Ctrl+Enter must execute only the selection; with no selection it executes the complete active document. The browser smoke path also clicks **Explain**, confirms a plan result and the non-execution toast, opens **Plan**, collapses and re-expands a child operator, and verifies cost/row metrics when present. On PostgreSQL/MySQL it confirms **Analyze** shows an execution warning before running a disposable SELECT and that SQLite/SQL Server/Oracle report the capability boundary. Verify the action is disabled when the driver lacks `explain`. The DDL smoke path selects a routine or trigger, confirms **Edit in SQL** creates a new tab without a result, verifies **Run in Transaction** is available, and uses **Refresh metadata** after the definition changes.
...
The result-grid smoke path clicks a cell, Shift-clicks another cell, presses Cmd/Ctrl+C, and pastes into a spreadsheet or plain-text editor to verify rectangular TSV output. It repeats the check with a row-number selection and the **NULL** display toggle, resizes a column by mouse and keyboard, then confirms Copy uses only the current filtered/sorted page when no range is selected, page navigation resets after filtering/sorting, and Export includes all loaded filtered/sorted rows. For a keyed table, it uses **Browse data**, loads another 100 rows, stages a non-key edit, confirms the generated UPDATE preview contains original-value predicates, verifies Cancel and Discard do not write, and applies the change only through the explicit native edit-batch transaction. For native streaming, it selects a 10,000-row cap, confirms the rows-loaded status grows as chunks arrive, verifies a result exceeding the cap retains only the bounded prefix with a successful limit warning, confirms PostgreSQL/MySQL cancellation cleans up the event listener, and verifies SQLite remains non-cancellable. Native contract tests also force a conflict and verify the entire SQLite batch rolls back.

The Data Compare smoke path selects a primary-key table, opens Cmd/Ctrl+K → **Compare selected table data**, chooses a same-driver target, confirms source/target counts and INSERT/UPDATE/DELETE rows, clears one change, opens the generated SQL preview, and verifies no target write occurred during comparison. On a disposable writable target, it applies the remaining changes and confirms the target row count and values. Repeat with a read-only target to verify preview remains available while Apply is disabled, and mutate a target row between compare and apply to verify the optimistic predicate rolls the batch back.
...
The PostgreSQL contract test is environment-selective. Set `QUERYX_TEST_POSTGRES_DATABASE` plus any required host, port, username, and password variables before `cargo test` to exercise a live disposable server. The MySQL/MariaDB contract is separately enabled with `QUERYX_TEST_MYSQL_DATABASE` and its optional host, port, username, and password variables; Oracle uses `QUERYX_TEST_ORACLE_SERVICE` plus optional host, port, user, and password variables. These contracts verify a read-only health query, metadata loading, and native write rejection. When database variables are absent, no external connection is attempted and the offline suite remains deterministic. See [PostgreSQL Driver](postgres-driver.md), [MySQL/MariaDB Driver](mysql-driver.md), and [Oracle Driver](oracle-driver.md).
...
- connection-manager profile save, duplicate, delete, isolated test, and reconnect flow; assert password fields are absent from persisted profile data, keychain presence markers do not contain secrets, native save/load/delete uses the OS credential store, and browser preview never invokes secret persistence
...
- explicit transaction begin/commit/rollback persistence, held-connection reuse for execute/stream/edit-batch, nested-session rejection, and disconnect rollback
