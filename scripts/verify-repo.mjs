...
const manifests = ['package.json', 'apps/desktop/package.json', 'packages/core/package.json', 'packages/shared/package.json'];
const errors = [];

if (!/^0\.\d+\.\d+$/.test(version) && !/^\d+\.\d+\.\d+$/.test(version)) errors.push(`VERSION is not valid semver: ${version}`);
...
for (const required of ['README.md', 'ROADMAP.md', 'CHANGELOG.md', 'docs/DOCUMENTATION_PLAN.md', 'docs/connections.md', 'docs/sql-editor.md', 'docs/postgres-driver.md', 'docs/testing.md', 'docs/release-process.md']) {
...
const markdownFiles = [join(root, 'README.md'), join(root, 'ROADMAP.md'), ...walk(join(root, 'docs')).filter((file) => extname(file) === '.md')];
const linkPattern = /\[[^\]]+\]\(([^)]+)\)/g;
for (const file of markdownFiles) {
  ...
  for (const match of content.matchAll(linkPattern)) {
    ...
    if (!existsSync(localTarget)) errors.push(`${file.replace(`${root}/`, '')} links to missing ${target}`);
  }
}
...
console.log(`✓ ${markdownFiles.length} Markdown files checked for local link integrity`);
