...
describe('inspectQuerySafety', () => {
  ...
  it('flags DELETE statements after removing comments', () => {
    expect(inspectQuerySafety('-- review first\nDELETE FROM users;').isDangerous).toBe(true);
  });

  it('allows destructive statements with a WHERE clause', () => {
    expect(inspectQuerySafety('DELETE FROM users WHERE id = 1').isDangerous).toBe(false);
  });
...
describe('InMemoryDriver', () => {
  it('normalizes a query result and exposes metadata', async () => {
    ...
    await driver.connect({ kind: 'postgres', name: 'test' });
    const result = await driver.execute('SELECT * FROM orders');
    const database = await driver.metadata();

    expect(result.columns.map((column) => column.name)).toEqual(['day', 'orders', 'revenue']);
  ...
  it('honors an already-aborted signal', async () => {
    ...
    await expect(driver.execute('SELECT 1', controller.signal)).rejects.toMatchObject({ name: 'AbortError' });
  });
});
