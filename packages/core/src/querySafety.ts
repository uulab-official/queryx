...
export function inspectQuerySafety(sql: string): QuerySafetyReport {
  ...
  if (!operationMatch) return { isDangerous: false, reason: 'No destructive operation detected' };
  const operation = operationMatch[1].toUpperCase() as 'UPDATE' | 'DELETE';
  if (/\bWHERE\b/i.test(normalized)) return { isDangerous: false, operation, reason: 'WHERE clause detected' };
