...
function App() {
  ...
  const handleRun = (mode: RunMode = 'normal') => {
    if (mode === 'normal') {
      ...
      if (safety.isDangerous) { setPendingSafety(safety); return; }
    }
  ...
  useEffect(() => {
    const listener = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') { event.preventDefault(); handleRun(); }
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <aside className="sidebar">
        ...
        <div className="recent-list">{history.length > 0 ? history.slice(0, 3).map((entry) => <Recent key={entry.id} name={entry.label} time={relativeTime(entry.executedAt)} error={entry.status === 'error'} onClick={() => setSql(entry.sql)} />) : <><Recent name="Daily revenue" time="2 minutes ago" /><Recent name="Active subscriptions" time="Yesterday" /><Recent name="Migration check" time="Yesterday" error /></>}</div>
    ...
    {pendingSafety && <SafeModeDialog report={pendingSafety} onCancel={() => setPendingSafety(null)} onRunInTransaction={() => handleRun('transaction')} onExecuteAnyway={() => handleRun('execute-anyway')} />}
  </div>;
}
...
function Recent({ name, time, error = false, onClick }: { name: string; time: string; error?: boolean; onClick?: () => void }) {
  return <button className="recent-query" onClick={onClick}><span className={`query-status ${error ? 'error' : 'success'}`}>{error ? '×' : '✓'}</span><span><strong>{name}</strong><small>{time}</small></span></button>;
}

function relativeTime(value: string): string {
  const minutes = Math.max(1, Math.round((Date.now() - new Date(value).getTime()) / 60_000));
  return minutes < 60 ? `${minutes} minute${minutes === 1 ? '' : 's'} ago` : 'Earlier today';
}

function SafeModeDialog({ report, onCancel, onRunInTransaction, onExecuteAnyway }: { report: QuerySafetyReport; onCancel: () => void; onRunInTransaction: () => void; onExecuteAnyway: () => void }) {
  return <div className="modal-backdrop" role="presentation"><section className="safe-modal" role="alertdialog" aria-modal="true" aria-labelledby="safe-mode-title"><div className="danger-icon">!</div><div><p className="modal-kicker">SAFE MODE</p><h2 id="safe-mode-title">Dangerous query detected</h2><p className="modal-copy">{report.operation} has no WHERE clause. This could affect every matching row in the table.</p><div className="affected-rows"><span>Estimated affected rows</span><strong>1,248,521</strong></div><div className="modal-actions"><button className="modal-secondary" onClick={onCancel}>Cancel</button><button className="modal-transaction" onClick={onRunInTransaction}>Run in Transaction</button><button className="modal-danger" onClick={onExecuteAnyway}>Execute Anyway</button></div></div></section></div>;
}
