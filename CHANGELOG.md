...
- Added native SQLite workspace persistence with WAL-backed schema migrations, atomic snapshot/profile upserts, legacy JSON/browser migration, and a strict no-password storage boundary.
- Added same-driver primary-key table Data Compare with bounded reads, selectable INSERT/UPDATE/DELETE synchronization SQL, optimistic target predicates, read-only target protection, and transactional apply.
