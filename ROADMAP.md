...
- [x] Table creation, add-column, column rename, type/nullability/non-PK-drop, ordered UNIQUE/non-unique index-create, named UNIQUE/CHECK constraint create, regular index-drop, guarded index rename, named foreign-key add/drop, view create/alter/drop, and routine/trigger definition forms with validation, SQL preview, explicit transaction apply, driver-aware manual review, dependency warnings, and metadata refresh; broader index alteration and multi-object routine/trigger forms remain planned
